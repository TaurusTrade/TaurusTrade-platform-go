...
// This macro is used to determine whether a specified iterator is an output iterator
// by ensuring that the iterator_category iterator_trait is or inherits from the
// output_iterator_tag. We use a static_assert to emit a compilation error when the 
// iterator does not meet this requirement. This macro is used for verifying that 
// the destination iterator passed to the concurrency::copy overloads is of the right type.
#define _CPP_AMP_VERIFY_OUTPUT_ITERATOR(_Type_name) \
    static_assert(std::is_base_of<std::output_iterator_tag, typename std::iterator_traits<_Type_name>::iterator_category>::value, "Invalid destination argument type to concurrency::copy.")
